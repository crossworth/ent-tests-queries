...
func (nc *NodeCreate) SetFrom(s string) *NodeCreate {
	nc.mutation.SetFrom(s)
	return nc
}

// SetTo sets the "to" field.
func (nc *NodeCreate) SetTo(s string) *NodeCreate {
	nc.mutation.SetTo(s)
	return nc
}
...
func (nc *NodeCreate) check() error {
	if _, ok := nc.mutation.From(); !ok {
		return &ValidationError{Name: "from", err: errors.New(`ent: missing required field "Node.from"`)}
	}
	if _, ok := nc.mutation.To(); !ok {
		return &ValidationError{Name: "to", err: errors.New(`ent: missing required field "Node.to"`)}
	}
	return nil
}
...
func (nc *NodeCreate) createSpec() (*Node, *sqlgraph.CreateSpec) {
	...
	if value, ok := nc.mutation.From(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			...
			Value:  value,
			Column: node.FieldFrom,
		})
		_node.From = value
	}
	if value, ok := nc.mutation.To(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: node.FieldTo,
